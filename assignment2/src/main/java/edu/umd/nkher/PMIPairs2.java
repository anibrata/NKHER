...
public class PMIPairs2 extends Configured implements Tool {

  private static final Logger LOG = Logger.getLogger(PMIPairs2.class);

  // First stage Mapper: emits pairs (token, 1) for each unique token in an
  // input value,
  ...
  // Second stage mapper: Maps (A, TOTAL_A), (B, TOTAL_B) and (A_B, TOTAL_A_B)
  // to the same reducer
  // via a common key
  ...
  private static class PairsPMIReducer extends
    ...
    private static Map<String, Integer> termTotals =
    ...
    public void setup(Context context) throws IOException {
      ...
      Path inFile =
          new Path(
              "/home/cloudera/Desktop/Assignments/bigdata-assignments/assignment2/appearance_totals_pairs/part-r-00000");
  ...
  public int run(String[] args) throws Exception {
    ...
    // The fits job will have an intermediate output path from which the second
    // job's reducer will read
    String outputPath = cmdline.getOptionValue(OUTPUT);
    String intermediatePath =
        "/home/cloudera/Desktop/Assignments/bigdata-assignments/assignment2/appearance_totals_pairs";

    int reduceTasks =
        cmdline.hasOption(NUM_REDUCERS) ? Integer.parseInt(cmdline
            .getOptionValue(NUM_REDUCERS)) : 1;

    LOG.info("Tool: " + PMIPairs2.class.getSimpleName() + " Appearances Part");
    ...
    job1.setJobName(PMIPairs2.class.getSimpleName() + " AppearanceCount");
    job1.setJarByClass(PMIPairs2.class);
    ...
    LOG.info("Tool: " + PMIPairs2.class.getSimpleName() + " Pairs Part");
    LOG.info(" - input path: " + inputPath);
    LOG.info(" - output path: " + outputPath);
    LOG.info(" - number of reducers: " + reduceTasks);

    Job job2 = Job.getInstance(conf);
    job2.setJobName(PMIPairs2.class.getSimpleName() + " PairsPMICalcuation");
    job2.setJarByClass(PMIPairs2.class);
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new PMIPairs2(), args);
  }
}